...
# Minimal stub so early @api_v1_bp.route decorators don't fail before Flask app is created
...
@api_v1_bp.route('/api/extension/login', methods=['POST'])
def extension_login():
    try:
...
@api_v1_bp.route('/api/desktop/login', methods=['POST'])
def desktop_login():
...
@api_v1_bp.route('/api/desktop/generate-token', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/desktop/notifications')
...
@api_v1_bp.route('/api/desktop/check-update', methods=['GET'])
...
@api_v1_bp.route('/api/desktop/download-update', methods=['GET'])
...
@api_v1_bp.route('/api/notifications')
...
@api_v1_bp.route('/api/notifications/<int:notif_id>/hide', methods=['POST'])
...
pass # app = Flask(__name__, static_folder='frontend/dist', static_url_path='/static', instance_relative_config=True)
...
@api_v1_bp.route('/api/test-openai-connection', methods=['POST', 'GET'])
@login_required
...
@api_v1_bp.route('/api/test-zai-connection', methods=['POST', 'GET'])
@login_required
...
@api_v1_bp.route('/api/test-perplexity-connection', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/test-gemini-connection', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/test-ai-connection-generic', methods=['POST'])
@login_required
...
@api_v1_bp.route("/api/login", methods=["POST"])
def api_login():
...
@api_v1_bp.route("/api/logout", methods=["POST"])
@login_required
def api_logout():
...
@api_v1_bp.route('/api/ai/market-analysis-workflow-prompt', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/risk-assessment-workflow-prompt', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/portfolio-review-workflow-prompt', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/workflow-latest', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/force-sentiment-analysis', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/logs/sync', methods=['POST'])
@login_required
def api_logs_sync():
...
@api_v1_bp.route('/api/sync-portfolio-from-transactions', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/logs/import', methods=['POST'])
@login_required
def api_logs_import():
...
@api_v1_bp.route('/logs.html')
@login_required
def logs_html():
    return jsonify({"error": "Logs page not available in React app"}), 404

@api_v1_bp.route('/register', methods=['GET', 'POST'])
def register():
...
@api_v1_bp.route('/onboarding', methods=['GET', 'POST'])
@login_required
def onboarding():
    if request.method == 'POST':
...
@api_v1_bp.route('/api/check-credential')
@login_required
...
@api_v1_bp.route('/api/get-credentials')
@login_required
...
@api_v1_bp.route("/api/sync-coins", methods=["POST"])
@login_required
def api_sync_coins():
    """
...
@api_v1_bp.route("/reset-password", methods=["GET", "POST"])
@login_required
def reset_password():
    if request.method == "POST":
        password = request.form.get("password")
...
@api_v1_bp.route("/api/unhide-all", methods=["POST"])
@login_required
def unhide_all():
    data = request.get_json()
...
@api_v1_bp.route("/dashboard.html")
@login_required
def dashboard_html():
    record_true_portfolio_value()
    # Serve the React app
...
@api_v1_bp.route('/api/tax/manual-investment', methods=['GET', 'POST'])
@login_required
...
@api_v1_bp.route('/api/tax-report')
@login_required
def api_tax_report():
...
@api_v1_bp.route('/api/transactions', methods=['POST'])
@login_required
def add_transaction():
...
@api_v1_bp.route("/api/true-portfolio-history")
@login_required
...
@api_v1_bp.route("/api/record-portfolio-value", methods=["POST"])
@login_required
...
@api_v1_bp.route("/test-session")
@login_required
def test_session():
    return f"Logged in as: {getattr(current_user, 'username', None)}"
@api_v1_bp.route("/api/hide-coin", methods=["POST"])
@login_required
def hide_coin():
    data = request.get_json()
...
@api_v1_bp.route("/api/set-favorite", methods=["POST"])
@login_required
def set_favorite():
    data = request.get_json()
    coin_id = data.get("id")
...
@api_v1_bp.route("/api/pionex-price")
@login_required
...
@api_v1_bp.route("/api/binance-price")
@login_required
...
@api_v1_bp.route("/api/chart_history/<symbol>")
@login_required
...
@api_v1_bp.route("/api/coingecko_chart/<slug>")
...
@api_v1_bp.route("/api/logs/taxable")
@login_required
...
@api_v1_bp.route("/api/coin-data-live")
@login_required
...
@api_v1_bp.route("/api/portfolio-history")
@login_required
...
@api_v1_bp.route("/api/update-note", methods=["POST"])
@login_required
def api_update_note():
...
@api_v1_bp.route("/api/set-initial-price", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/delete-coin", methods=["POST"])
@login_required
def api_delete_coin():
    data = request.get_json()
    coin_id = data.get("id")
...
@api_v1_bp.route("/api/set-custom-pct", methods=["POST"])
@login_required
def set_custom_pct():
    data = request.get_json()
    coin_id = data.get("id")
...
@api_v1_bp.route("/api/set-alert", methods=["POST"])
@login_required
def set_alert():
    data = request.get_json()
...
@api_v1_bp.route("/api/set-custom-pct-type", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/clear-alert-state", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/debug-alerts")
@login_required
def debug_alerts():
...
@api_v1_bp.route("/api/alert-status", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/auto-alert")
@login_required
def api_auto_alert():
...
@api_v1_bp.route("/api/watchlist-live")
@login_required
...
@api_v1_bp.route("/api/watchlist/add", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/watchlist/remove", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/hidden-coins")
@login_required
...
@api_v1_bp.route("/api/staking/assets", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/stakeable-coins", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/stake", methods=["POST"])
@login_required
def api_stake_asset():
...
@api_v1_bp.route("/api/staking/unstake", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/dust/assets", methods=["GET"])
@login_required
def api_dust_assets():
...
@api_v1_bp.route("/api/dust/convert", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/dust/history", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/balance", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/history", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/rewards", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/dashboard-summary", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/dashboard-summary-live", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/dashboard-summary-dashboard", methods=["GET"])
@api_v1_bp.route("/api/staking/dashboard-summary-dashboard/<path:cache_buster>", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/staking/dashboard-view", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/true-portfolio-value")
@login_required
...
@api_v1_bp.route("/api/true-portfolio-value-live")
@login_required
...
@api_v1_bp.route("/api/set-watch-alert", methods=["POST"])
@login_required
def set_watch_alert():
    data = request.get_json()
    symbol = data.get("symbol", "").upper()
...
@api_v1_bp.route("/api/set-watch-alert-type", methods=["POST"])
@login_required
...
@api_v1_bp.route('/api/set-volatility-pct', methods=['POST'])
@login_required
...
@api_v1_bp.route("/api/set-watchlist-favorite", methods=["POST"])
@login_required
...
@api_v1_bp.route("/trading")
@login_required
def trading_page():
    """Serve the trading page"""
    return serve_react_app()

@api_v1_bp.route("/logs")
@login_required
def logs_page():
    """Serve the logs page"""
    return serve_react_app()

@api_v1_bp.route("/portfolio")
@login_required
def portfolio_page():
...
@api_v1_bp.route("/ai-dashboard")
@login_required
def ai_dashboard_page():
    """Serve the AI dashboard page (legacy route)"""
    return serve_react_app()

@api_v1_bp.route("/ai-analysis")
@login_required
def ai_analysis_page():
    """Serve the AI Analysis page (new route)"""
    return serve_react_app()

@api_v1_bp.route("/staking")
@login_required
def staking_page():
    """Serve the staking page"""
    return serve_react_app()

@api_v1_bp.route("/tax-report")
@login_required
def tax_report_page():
    """Serve the tax report page"""
    return serve_react_app()

@api_v1_bp.route("/help")
@login_required
def help_page():
    """Serve the help page"""
    return serve_react_app()

@api_v1_bp.route("/api/ai/settings", methods=["GET", "POST"])
@login_required
def api_ai_settings():
...
@api_v1_bp.route("/api/check-trade-permission")
@login_required
...
@api_v1_bp.route("/api/mark-onboarding-complete", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/support/send", methods=["POST"])
...
@api_v1_bp.route("/api/tax-report/export", methods=["GET"])
@login_required
...
@api_v1_bp.route("/api/account/delete", methods=["DELETE"])
@login_required
def delete_account():
    try:
        user_id = current_user.id
...
@login_required
@api_v1_bp.route("/api/settings", methods=["GET", "POST"])
@login_required
def api_settings():
...
@api_v1_bp.route('/api/ai/models', methods=['GET'])
@login_required
def get_ai_models():
...
@api_v1_bp.route("/api/test-binance-connection", methods=["GET", "POST"])
@login_required
...
@api_v1_bp.route("/api/test-trading-connection", methods=["POST"])
@login_required
...
@api_v1_bp.route("/api/test-openai-connection")
@login_required
...
@api_v1_bp.route("/api/test-zai-connection")
@login_required
...
@api_v1_bp.route("/api/test-brave-search", methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading-pairs')
@login_required
...
@api_v1_bp.route('/api/market-data/<symbol>')
@login_required
...
@api_v1_bp.route('/api/place-order', methods=['POST'])
@login_required
def api_place_order():
...
@api_v1_bp.route('/api/sync-portfolio', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/transaction-history')
@login_required  
...
@api_v1_bp.route('/api/pending-orders')
@login_required
...
@api_v1_bp.route('/api/portfolio-analysis')
@login_required
...
@api_v1_bp.route('/api/cancel-order/<order_id>', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/order-status/<order_id>')
@login_required
...
@api_v1_bp.route('/api/trading/settings', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/settings', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/order-types', methods=['GET'])
    ...
    try:
        ...
        if symbol:
            try:
                ...
                if api_key and api_secret:
                    client = BClient(api_key=api_key, api_secret=api_secret, testnet=False, tld='us')
...
@api_v1_bp.route('/api/trading/klines/<symbol>', methods=['GET'])
...
@api_v1_bp.route('/api/trading/transactions/<symbol>', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/symbol-info/<symbol>', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/test-order', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/orders', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/real-orders', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/2fa/setup', methods=['POST'])
@login_required
def setup_2fa():
...
@api_v1_bp.route('/api/trading/2fa/verify-setup', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/2fa/disable', methods=['POST'])
@login_required
def disable_2fa():
...
@api_v1_bp.route('/api/trading/2fa/verify', methods=['POST'])
@login_required
def verify_2fa_code():
...
@api_v1_bp.route('/api/trading/portfolio', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/portfolio/backfill', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/test-orders', methods=['GET'])
@login_required
def get_test_orders():
...
@api_v1_bp.route('/api/trading/place-order', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/fees/<symbol>', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/price/<symbol>', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/balances/<symbol>', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/trading/open-orders', methods=['GET'])
@login_required
def get_open_orders():
    """Get all open orders"""
    try:
...
@api_v1_bp.route('/api/trading/test-oco-order', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/trading/oco-order', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/ai/market-analysis')
@login_required
...
@api_v1_bp.route('/api/ai/recommendations')
@login_required
...
@api_v1_bp.route('/api/ai/portfolio-analysis')
@login_required
...
@api_v1_bp.route('/api/ai/market-analysis/<symbol>')
@login_required
...
@api_v1_bp.route('/api/ai/smart-alerts')
@login_required
...
@api_v1_bp.route('/api/ai/recommendation-score/<symbol>')
@login_required
...
@api_v1_bp.route('/api/debug/background-jobs', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/conversations')
@login_required
...
@api_v1_bp.route('/api/ai/conversation', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/ai/conversations/<int:message_id>', methods=['DELETE'])
@login_required
...
@api_v1_bp.route('/api/ai/conversations/<int:message_id>/archive', methods=['PATCH'])
@login_required
...
@api_v1_bp.route('/api/ai/news-analysis', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/ai/run-analysis', methods=['POST'])
@login_required
...
@api_v1_bp.route('/api/ai/coin-analysis', methods=['GET', 'POST'])
@login_required
...
@api_v1_bp.route('/api/widgets/fear-greed', methods=['GET'])
...
@api_v1_bp.route('/api/widgets/cbbi', methods=['GET'])
def api_cbbi_data():
...
@api_v1_bp.route('/api/ai/market-analysis-workflow', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/risk-assessment-workflow', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/portfolio-review-workflow', methods=['GET', 'POST'])
@login_required
...
@api_v1_bp.route('/api/ai/portfolio-review-results', methods=['GET'])
@login_required
...
@api_v1_bp.route('/api/ai/copilot-results', methods=['GET'])
@login_required
...
if __name__ == "__main__":
    pass # app.run(host='0.0.0.0', port=5010, debug=False, use_reloader=False)
def calculate_symbol_snapshot(symbol):
    """Compute technical snapshot and trading signal for a symbol based on recent prices."""
    try:
        price_data = get_last_7d_prices(symbol)
        if not price_data or len(price_data) < 2:
            return None
        ...
        technical_score = 0
        if sma_relation == 'Above':
            technical_score += 25
        if rsi is not None and 40 <= rsi <= 70:
            technical_score += 20
        if pct_1d and pct_1d > 0:
            technical_score += 15
        if pct_3d and pct_3d > 0:
            technical_score += 15
        if macd_val >= 0:
            technical_score += 15
        ...
        entry_price = current_price
        stop_loss = round(entry_price * (0.95 if signal == 'BUY' else 1.05), 2)
        take_profit = round(entry_price * (1.15 if signal == 'BUY' else 0.85), 2)

        patterns = []
        if price_change > 10:
            patterns.append("Strong uptrend detected")
        elif price_change < -10:
            patterns.append("Strong downtrend detected")
        elif abs(price_change) < 3:
            patterns.append("Sideways consolidation")

        risk_factors = []
        if volatility > 0.15:
            risk_factors.append("High volatility may increase risk")
        if abs(price_change) > 20:
            risk_factors.append("Large price movement may indicate overbought/oversold conditions")
        if len(price_data) < 7:
        ...
        return {
            ...
            "technical_indicators": {
                "rsi": round(rsi, 1) if rsi is not None else None,
                "macd": macd_signal,
                "sma7_relation": sma_relation,
                "mom_1d_pct": round(pct_1d, 2) if pct_1d is not None else None,
                "mom_3d_pct": round(pct_3d, 2) if pct_3d is not None else None,
                "mom_7d_pct": round(pct_7d, 2) if pct_7d is not None else None,
