...
def encrypt_secret(value: Optional[str]) -> Optional[str]:
    ...
    Returns None when the input is falsy to mirror previous behaviour
    that stored NULL for missing keys.
    """
    if not value:
        return None
    fernet = _get_fernet()
...
def decrypt_secret(value: Optional[str]) -> Optional[str]:
    """
    Attempt to decrypt a secret value. Falls back to returning the original
    value when the key is missing or the value was stored in plaintext.
    """
    if not value:
        return None
    try:
