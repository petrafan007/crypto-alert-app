...
- **Startup migration repair:** Corrected the watchlist table used to recover interrupted sentiment checks and removed a hardcoded GRAM acquisition-price assignment. Startup recovery now preserves unknown acquisition costs.
...
- **Fix (Dashboard):** The "Trading cutoff" column time in Positions now uses the highest-precision date embedded in the symbol to display real-time sub-minute accuracy, and the "Time to cutoff" column now displays seconds remaining (`#h #m #s`).
...
- Repaired specialist AI routing so module and master reports use the isolated paper ledger, saved strategy parameters, actual signal checks, exchange-session times, per-module results, and explicit engine-purpose instructions. The master uses recorded operational summaries rather than treating specialist prose as facts. Reports distinguish allocation budgets from buy signals, purchased YES/NO sides from settlement results, and percentage values from fractions.
...
- **Event Contract Reference Price Fallback**:
  - Resolves a provider reference price from Webull's supplied contract condition when the market response omits a structured `reference_price` or `target_value`, ensuring the Event price card remains populated for current Crypto contracts.
...
- **Usable Mobile Event Ticket**:
  - Reduces nested mobile ticket spacing so Event Contract filters, including the full Duration / Frequency value, retain a readable control width on narrow screens.
