...
class EventAlgoTests(unittest.TestCase):
    def test_audit_deferral_is_informational_and_retries_without_failure_backoff(self):
        ...
        with patch('event_algo.User') as users, \
                ...
                patch('event_algo._record_ai_batch_call'), \
            ...
            result = _predict_event_markets_batch(1, [market], config=SimpleNamespace())['TEST']
        ...
        row = SimpleNamespace(attempts=3, consecutive_failures=2, last_attempt_at=now-timedelta(hours=1))
        with patch('event_algo._event_ai_evaluation', return_value=row), patch('event_algo.db'):
            _record_ai_evaluation(1, 1, market, 'DAILY', result, {'scan_interval_seconds': 60}, now)
        self.assertEqual(row.status, 'SKIPPED')
        self.assertEqual((row.attempts, row.consecutive_failures), (3, 2))
        self.assertIsNone(row.next_retry_at)
        self.assertEqual(row.next_evaluation_at, now+timedelta(seconds=60))
        self.assertEqual(row.last_attempt_at, now-timedelta(hours=1))
