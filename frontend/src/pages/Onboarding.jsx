...
const PAGE_STEP = {
  ...
  'ai-secondary': 3, 'ai-tertiary': 3, 'ai-quaternary': 3, 'search-news': 3, telegram: 4, review: 5,
};
const BACK_PAGE = {
  ...
  'ai-tertiary': 'ai-secondary', 'ai-quaternary': 'ai-tertiary', 'search-news': 'ai-choice', telegram: 'search-news', review: 'telegram',
};
...
export default function Onboarding({ isLightMode, toggleTheme }) {
  ...
  const [ai, setAi] = useState({
    ...
    quaternary: { provider: '', model: '', reasoning: 'medium', apiKey: '' },
  });
  ...
  const testAndSaveAi = async (tier) => {
    ...
    try {
      ...
      const suffix = tier === 'primary' ? '' : tier === 'secondary' ? '_fallback' : tier === 'tertiary' ? '_tertiary' : '_quaternary';
  ...
  const afterAiTier = (tier) => go(tier === 'primary' ? 'ai-secondary' : tier === 'secondary' ? 'ai-tertiary' : tier === 'tertiary' ? 'ai-quaternary' : 'search-news');
