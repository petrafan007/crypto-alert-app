...
function BinanceLogo() {
  return <svg className="ob-exchange-logo binance" viewBox="0 0 64 64" aria-label="Binance.US"><g fill="currentColor"><path d="M32 7l9 9-9 9-9-9 9-9zM16 23l9 9-9 9-9-9 9-9zM48 23l9 9-9 9-9-9 9-9zM32 39l9 9-9 9-9-9 9-9z"/><path d="M32 21l11 11-11 11-11-11 11-11z"/></g></svg>;
}
function WebullLogo() {
  return <svg className="ob-exchange-logo webull" viewBox="0 0 64 64" aria-label="Webull"><path d="M10 37c10-18 28-26 44-19-11 1-20 6-26 15 8-5 16-6 24-3-9 3-16 9-21 18-8 0-15-4-21-11z" fill="currentColor"/><path d="M19 39c8 5 16 5 25 1-5 8-12 12-21 10l-4-11z" fill="currentColor" opacity=".55"/></svg>;
}
...
export default function Onboarding({ isLightMode, toggleTheme }) {
  ...
  const finish = async () => {
    setBusy(true); setMessage(null);
    try { await axios.post('/api/onboarding/finish', {}, { withCredentials: true }); await checkAuthStatus(); navigate('/', { replace: true }); }
  ...
  if (page === 'exchanges') { title='Choose at least one exchange'; subtitle='Connect Binance.US, Webull, or both before entering the Dashboard.'; body=<><Notice type="warning"><strong>Required:</strong> At least one exchange must pass its connection test.</Notice><div className="ob-choices three"><Choice selected={exchangeChoice==='binance'} onClick={() => setExchangeChoice('binance')} icon={<BinanceLogo/>} title="Binance.US">Connect spot balances, portfolios, orders, and market data.</Choice><Choice selected={exchangeChoice==='webull'} onClick={() => setExchangeChoice('webull')} icon={<WebullLogo/>} title="Webull">Connect Production OpenAPI, then select detected live or paper accounts.</Choice><Choice selected={exchangeChoice==='both'} onClick={() => setExchangeChoice('both')} icon={<span className="ob-both"><BinanceLogo/><WebullLogo/></span>} title="Both exchanges">Set up Binance.US first, followed by Webull after.</Choice></div></>; primary=continueExchangeChoice; primaryLabel=exchangeChoice==='webull'?'Continue to Webull':'Continue to Binance.US'; }
  ...
  return <div className="ob-page"><header className="ob-header"><BrandIcon/><span><strong>Crypto &amp; Securities Dashboard</strong><small>CSDapp.online</small></span><ThemeButton isLightMode={isLightMode} toggleTheme={toggleTheme}/></header><main className="ob-shell"><aside className="ob-rail"><h2>Welcome to Crypto &amp; Securities Dashboard</h2><p>Let’s configure your secure trading workspace.</p>{RAIL_STEPS.map((name,index)=><div key={name} className={`ob-step ${index<step?'done':''} ${index===step?'active':''}`}><span>{index<step?'✓':index+1}</span>{name}</div>)}<div className="ob-resume"><strong>Safe to leave</strong><small>Progress is saved after each completed step. Signing back in resumes at the first incomplete requirement.</small></div></aside><section className="ob-card"><div className="ob-progress"><i style={{width:`${((step+1)/6)*100}%`}}/></div><div className="ob-card-head"><small>Step {step+1} of 6</small><h1>{title}</h1><p>{subtitle}</p></div><div className="ob-card-body">{message&&<Notice type={message.type}>{message.text}</Notice>}{body}</div><footer className="ob-footer"><button className="ob-button link" onClick={logout}>Exit setup</button>{BACK_PAGE[page]&&<button className="ob-button secondary" onClick={back} disabled={busy}>Back</button>}<span/>{auxiliary}{primary&&<button className="ob-button primary" onClick={primary} disabled={busy}>{busy?'Working…':primaryLabel}</button>}</footer></section></main>{skip2faModal&&<div className="ob-modal-backdrop"><div className="ob-modal"><small>Security reminder</small><h2>Live trading will remain disabled</h2><p>You may complete onboarding without two-factor authentication, but you cannot place or cancel live trades until 2FA is configured and verified. This requirement helps protect your connected accounts and assets.</p><Notice type="warning">Paper trading remains available if you connect an eligible Webull paper account.</Notice><div><button className="ob-button secondary" onClick={()=>{setSkip2faModal(false);start2fa();}}>Set Up 2FA</button><button className="ob-button primary" onClick={defer2fa}>I Understand — Continue</button></div></div></div>}</div>;
}
