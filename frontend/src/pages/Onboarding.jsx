...
export default function Onboarding({ isLightMode, toggleTheme }) {
  ...
  const loadWebullAccounts = async () => {
    ...
    setAccounts(data.accounts || []); setEnabledAccounts(ids); setWebullStage('accounts'); await go('webull-accounts');
  };
  ...
  if (page === 'webull-accounts') { title='Choose Webull accounts'; subtitle='Select the Webull accounts you want to use in Crypto and Securities Dashboard.'; body=<>{accounts.map(a=>{const id=String(a.account_id);return <div className="ob-account" key={id}><input type="checkbox" checked={enabledAccounts.includes(id)} onChange={e=>{if(e.target.checked){setEnabledAccounts([...enabledAccounts,id]);}else{setEnabledAccounts(enabledAccounts.filter(x=>x!==id));}}}/><span><strong>{a.account_label||a.account_name||'Webull Account'}</strong><small>{a.account_id_masked}</small></span><span className={`ob-badge ${(a.account_type||'').toLowerCase().includes('paper')?'paper':''}`}>{a.account_type||a.account_class||'ACCOUNT'}</span></div>})}<Notice type="info">Enabled accounts automatically synchronize balances, positions, orders, and account activity. Selecting an account does not place trades.</Notice><Notice type="warning">Live trading remains locked until 2FA is enabled.</Notice></>; primary=saveWebullAccounts; primaryLabel='Save Accounts & Continue'; }
