import { buildSyntheticPayload, strategySummary, buildSyntheticReview } from '../utils/syntheticOrders.mjs';
...
function LegacyWebullOrderTable({ orders, emptyText, onCancelOrder, cancellingId, optionClosePnlByOrder = null }) {
...
function WebullOrderTable({ orders, emptyText, onCancelOrder, onReplaceOrder = null, cancellingId, optionClosePnlByOrder = null, userId, tableId, accounts = [] }) {
  ...
  const closePnlCell = (order) => {
    ...
    const closePnl = optionClosePnlByOrder?.[order.id];
    if (closePnl?.status === 'loading') return <span className="option-close-pnl-muted">Refreshing…</span>;
    if (closePnl?.status === 'not_filled') return <span className="option-close-pnl-muted" title="P&L requires an actual fill price and filled quantity.">Not filled</span>;
    if (closePnl?.status !== 'available') return <span className="option-close-pnl-muted" title={closePnl?.message || 'A current executable quote is unavailable.'}>Unavailable</span>;
  ...
  const renderClosePnl = (order) => {
    if (!option(order).isOption) return '—';
    const closePnl = optionClosePnlByOrder?.[order.id];
    if (closePnl?.status === 'loading') return <span className="option-close-pnl-muted">Refreshing…</span>;
    if (closePnl?.status === 'not_filled') return <span className="option-close-pnl-muted">Not filled</span>;
    if (closePnl?.status !== 'available') return <span className="option-close-pnl-muted" title={closePnl?.message || 'A current executable quote is unavailable.'}>Unavailable</span>;
    return <span className={closePnl.pnl >= 0 ? 'option-close-pnl-positive' : 'option-close-pnl-negative'}>{closePnl.pnl >= 0 ? '+' : '-'}${number(Math.abs(closePnl.pnl), 2)}<small>@ {closePnl.closeSide} ${number(closePnl.closePrice, 4)}</small></span>;
  };
  const columns = [
...
function LegacyEventContractOpenOrders({ orders, onManageOrder }) {
...
function EventContractOpenOrders({ orders, onManageOrder, userId, accounts = [] }) {
  ...
  const columns = [
    ...
    { id: 'quantity', label: 'Order Qty', value: (order) => Number(order.quantity), render: (order) => number(order.quantity, 6) },
...
const isIndividualCashAccount = (acc) => {
  ...
  return (identity.includes('individual') && identity.includes('cash')) || identity.includes('individual_cash');
};
...
const normalizePaperAccountId = (id) => {
  if (!id) return '';
  const s = String(id).trim();
  if (s === 'TEST_EVENTS_ACCOUNT' || s === 'TEST_ACC_EVENTS') return 'TEST_ACC_EVENTS';
  if (s === 'TEST_CRYPTO_ACCOUNT' || s === 'TEST_ACC_CRYPTO') return 'TEST_ACC_CRYPTO';
  if (s === 'TEST_FUTURES_ACCOUNT' || s === 'TEST_ACC_FUTURES') return 'TEST_ACC_FUTURES';
  if (s === 'TEST_INDIVIDUAL_CASH' || s === 'TEST_ACC_INDIVIDUAL_CASH') return 'TEST_ACC_INDIVIDUAL_CASH';
  if (s === 'QUANT_EVENTS_ACCOUNT' || s === 'QUANT_ACC_EVENTS') return 'QUANT_ACC_EVENTS';
  if (s === 'QUANT_CRYPTO_ACCOUNT' || s === 'QUANT_ACC_CRYPTO') return 'QUANT_ACC_CRYPTO';
  if (s === 'QUANT_FUTURES_ACCOUNT' || s === 'QUANT_ACC_FUTURES') return 'QUANT_ACC_FUTURES';
  if (s === 'QUANT_INDIVIDUAL_CASH' || s === 'QUANT_ACC_INDIVIDUAL_CASH') return 'QUANT_ACC_INDIVIDUAL_CASH';
  return s;
};
...
const holdingForAccount = (holdings, symbol, accountId) => (
  holdings.find((holding) => (
    (String(holding?.account_id || '') === String(accountId || '')
      || (normalizePaperAccountId(holding?.account_id) && normalizePaperAccountId(holding?.account_id) === normalizePaperAccountId(accountId)))
    && holdingMatchesSymbol(holding, symbol)
  ))
);
...
export default function WebullTrading({ isLightMode = false , isEmbeddedReplaceMode = false, embeddedOrder = null, embeddedCoin = null, onEmbeddedClose = null, onEmbeddedSuccess = null}) {
  ...
  const [quantSummary, setQuantSummary] = useState(null);
  ...
  const [eventSearchSubmitted, setEventSearchSubmitted] = useState(false);
  ...
  const handleDepositFakeMoney = async (amt) => {
    if (!amt || amt <= 0) return;
    setDepositError('');
  ...
  const handleResetPaperAccount = async () => {
    ...
    try {
      ...
      setDepositError(err.response?.data?.message || 'Reset failed.');
    } finally {
      setDepositSubmitting(false);
    }
  };
  ...
  const load = async (forcedTestMode = null) => {
    ...
    try {
      ...
      if (testModeActive) {
        ...
        const initialTestAcc = urlAccountId
          ? normalizePaperAccountId(urlAccountId)
          : (targetType === 'CRYPTO')
            ? 'TEST_ACC_CRYPTO'
            : (targetType === 'EVENT')
              ? 'TEST_ACC_EVENTS'
  ...
  const allQuantAccounts = useMemo(() => {
    const list = Array.isArray(quantSummary?.accounts) && quantSummary.accounts.length > 0
      ? quantSummary.accounts
      : (Array.isArray(quantSummary?.sub_accounts) && quantSummary.sub_accounts.length > 0
        ? quantSummary.sub_accounts
  ...
  const activeAccount = useMemo(() => {
    if (isQuantMode) {
      const match = allQuantAccounts.find((a) => (
        a.account_id === selectedAccountId ||
        normalizePaperAccountId(a.account_id) === normalizePaperAccountId(selectedAccountId)
      ));
      const fallbackAcc = (selectedInstrumentType === 'EVENT')
        ? (allQuantAccounts.find(isEventAccount) || allQuantAccounts[0])
        : (selectedInstrumentType === 'CRYPTO')
          ? (allQuantAccounts.find(isCryptoAccount) || allQuantAccounts[0])
          : (selectedInstrumentType === 'FUTURES')
            ? (allQuantAccounts.find(isFuturesAccount) || allQuantAccounts[0])
            : (allQuantAccounts.find(isIndividualCashAccount) || allQuantAccounts[0]);
      const acc = match || fallbackAcc || DEFAULT_QUANT_SUB_ACCOUNTS[0];
      ...
      return {
        ...
        unrealized_profit_loss: quantSummary?.unrealized_pnl ?? 0,
        unrealized_profit_loss_rate: quantSummary?.initial_balance ? ((quantSummary?.unrealized_pnl ?? 0) / quantSummary.initial_balance) * 100 : 0,
      };
    }
    if (isTestMode) {
      const match = allTestAccounts.find((a) => (
        a.account_id === selectedAccountId ||
        normalizePaperAccountId(a.account_id) === normalizePaperAccountId(selectedAccountId)
      ));
      const fallbackAcc = (selectedInstrumentType === 'EVENT')
        ? (allTestAccounts.find(isEventAccount) || allTestAccounts[0])
        : (selectedInstrumentType === 'CRYPTO')
          ? (allTestAccounts.find(isCryptoAccount) || allTestAccounts[0])
          : (selectedInstrumentType === 'FUTURES')
            ? (allTestAccounts.find(isFuturesAccount) || allTestAccounts[0])
            : (allTestAccounts.find(isIndividualCashAccount) || allTestAccounts[0]);
      const acc = match || fallbackAcc || DEFAULT_TEST_SUB_ACCOUNTS[0];
  ...
  }, [isTestMode, isQuantMode, quantSummary, paperSummary, accounts, selectedAccountId, selectedInstrumentType, allQuantAccounts, allTestAccounts]);
  ...
  const handleAssetClassChange = (nextType) => {
    ...
    if (nextType === 'EVENT') {
      ...
      const currAcc = targetPool.find((a) => (
        a.account_id === selectedAccountId ||
        normalizePaperAccountId(a.account_id) === normalizePaperAccountId(selectedAccountId)
      ));
  ...
  const handleAssetTabChange = (nextType) => {
    ...
    handleAssetClassChange(nextType);
  };
  ...
  const cashBalance = useMemo(() => {
    if (isQuantMode) {
      ...
      return Number(quantSummary?.total_cash ?? quantSummary?.buying_power ?? 0);
    }
    if (isTestMode) {
      if (activeAccount?.cash_balance != null) {
        return Number(activeAccount.cash_balance);
  ...
  }, [isTestMode, isQuantMode, quantSummary, paperSummary, activeAccount]);
  ...
  const loadEventMarkets = async ({
    ...
    if (!category || !cleanQuery) return { loading: false, status: 'idle' };
    eventMarketAbortControllerRef.current?.abort();
    const controller = new AbortController();
    eventMarketAbortControllerRef.current = controller;
    const requestId = ++eventMarketRequestRef.current;
    setEventLoading(true);
    setEventMessage('');
    try {
      const response = await axios.get('/api/webull/events/markets', {
        params: { category, query: cleanQuery, duration, limit: 20, search: 1 },
        withCredentials: true,
        signal: controller.signal,
      });
      if (requestId !== eventMarketRequestRef.current || controller.signal.aborted) return { aborted: true };
      ...
      if (requestId !== eventMarketRequestRef.current || controller.signal.aborted || axios.isCancel(err)) return { aborted: true };
      setEventMarkets([]);
      ...
      if (eventMarketAbortControllerRef.current === controller) eventMarketAbortControllerRef.current = null;
      if (requestId === eventMarketRequestRef.current) setEventLoading(false);
    }
  };

  const loadEventCategories = async () => {
    setEventMetadataLoading(true);
    setEventMessage('');
    try {
      ...
      setEventTotalMatches(0);
      setEventHasMore(false);
      setEventSearchSubmitted(false);
      setEventMarketMenuOpen(false);
    } catch (err) {
      setEventMessage(err.response?.data?.message || err.message || 'Unable to load Webull Event Contract categories.');
    } finally {
      setEventMetadataLoading(false);
    }
  };

  const loadEventDurationOptions = async (category) => {
    if (!category) return;
    const requestId = ++eventMetadataRequestRef.current;
    setEventMetadataLoading(true);
    setEventMessage('');
    try {
      const response = await axios.get('/api/webull/events/durations', {
        params: { category },
        withCredentials: true,
      });
      if (requestId !== eventMetadataRequestRef.current) return;
      setEventDurationOptions(response.data?.duration_options || []);
    } catch (err) {
      if (requestId !== eventMetadataRequestRef.current) return;
      setEventDurationOptions([]);
      setEventMessage(err.response?.data?.message || 'Unable to load Webull Event Contract durations.');
    } finally {
      if (requestId === eventMetadataRequestRef.current) setEventMetadataLoading(false);
    }
  };
  ...
  const handleEventCategoryChange = (categoryCode) => {
    ...
    eventMarketAbortControllerRef.current?.abort();
    eventMarketAbortControllerRef.current = null;
    eventMetadataRequestRef.current += 1;
  ...
  const handleEventMarketQueryChange = (query) => {
    eventMarketRequestRef.current += 1;
    eventMarketAbortControllerRef.current?.abort();
    eventMarketAbortControllerRef.current = null;
    setEventMarketQuery(query);
    setEventMarkets([]);
    setEventTotalMatches(0);
    setEventHasMore(false);
    setEventMessage('');
  ...
  const submitEventMarketSearch = () => {
    const query = eventMarketQuery.trim();
  ...
  useEffect(() => {
    if (selectedInstrumentType === 'EVENT' && !eventCategories.length && !eventMetadataLoading) {
      loadEventCategories();
    }
  }, [selectedInstrumentType]);
  ...
  useEffect(() => {
    ...
    loadEventDurationOptions(selectedEventCategory);
    return () => {
      eventMetadataRequestRef.current += 1;
    };
  ...
  const currentHolding = useMemo(() => {
    ...
    if (selectedInstrumentType === 'EVENT') {
      return modeHoldings.find((holding) => (
        (String(holding?.account_id || '') === String(selectedAccountId || '')
          || (normalizePaperAccountId(holding?.account_id) && normalizePaperAccountId(holding?.account_id) === normalizePaperAccountId(selectedAccountId)))
        && String(holding?.instrument_type || '').toUpperCase() === 'EVENT'
        && holdingMatchesSymbol(holding, selectedSymbol)
        && String(holding?.event_outcome || '').toLowerCase() === String(orderForm.eventOutcome || '').toLowerCase()
      )) || null;
    }
  ...
  const webullTwoFactorOrderDetails = () => ({
    ...
    syntheticReview: ['LADDER', 'SYNTHETIC'].includes(orderForm.type) ? buildSyntheticReview(ladderConfig, { side: orderForm.side, quantity: orderForm.quantity, currentPrice: Number(livePrice), baseAsset: selectedSymbol, quantityStep: selectedInstrumentType === 'CRYPTO' ? .00000001 : .00001 }) : undefined,
  ...
  useEffect(() => {
    ...
    const ready = (
      selectedInstrumentType === 'EVENT'
      && (String(selectedAccountId || '') === pendingEventPositionOrder.accountId || normalizePaperAccountId(selectedAccountId) === normalizePaperAccountId(pendingEventPositionOrder.accountId))
  ...
  const handleComboSubmit = async (twofaToken = null) => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        if (isTestMode) {
          loadPaperTradingData();
          loadOpenOrders(selectedAccountId || 'TEST_ACC_INDIVIDUAL_CASH');
        } else if (isQuantMode) {
          loadQuantTradingData();
          loadOpenOrders(selectedAccountId || 'QUANT_ACC_INDIVIDUAL_CASH');
        } else {
          loadOpenOrders(selectedAccountId);
        }
        loadHistory(selectedAccountId);
  ...
  return (
    <div className="trading-page webull-trading-page">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '0.9rem', flexWrap: 'wrap' }}>
            ...
            <div>
              ...
              <strong style={{ color: isLightMode ? '#0f172a' : '#f8fafc' }}>${number(quantSummary?.total_equity ?? cashBalance)} USD</strong>
            </div>
            {quantSummary?.drawdown_pct != null && (
              <div>
                <span style={{ color: '#94a3b8', marginRight: '6px' }}>Drawdown:</span>
                <strong style={{ color: Number(quantSummary.drawdown_pct) > 5 ? '#f87171' : '#4ade80' }}>
                  {Number(quantSummary.drawdown_pct).toFixed(2)}%
                </strong>
              </div>
            )}
            <button
      ...
      <div className="trading-content">
        {loading ? (
          ...
          <>
              ...
              <div className="order-form-container">
                ...
                {!isEmbeddedReplaceMode && (selectedInstrumentType === 'FUTURES' ? (
                  <WebullFuturesLightweightChart
                    ...
                    selectedAccountId={displayAccounts.some((a) => a.account_id === selectedAccountId) ? selectedAccountId : (activeAccount?.account_id || selectedAccountId)}
                ...
                >
                  ...
                  <form onSubmit={handleOrderSubmit} className="trading-order-panel" noValidate>
                        ...
                        <div className="event-contract-selector-grid">
                          <div>
                            ...
                            <select
                              ...
                              disabled={eventMetadataLoading && !eventCategories.length}
                            >
                          ...
                          <div>
                            ...
                            <select
                              value={eventDuration}
                              onChange={(event) => handleEventDurationChange(event.target.value)}
                              className="order-styled-input"
                            >
                          ...
                          <div className="event-market-combobox" ref={eventMarketSelectorRef}>
                            ...
                            <div className="event-market-search-row">
                              <input
                                ...
                                onChange={(event) => handleEventMarketQueryChange(event.target.value)}
                                onKeyDown={(event) => {
                                  if (event.key === 'Enter') {
                                    event.preventDefault();
                                    submitEventMarketSearch();
                                  }
                                }}
                              ...
                              <button
                                ...
                                onClick={submitEventMarketSearch}
                                aria-label="Search Event Contract markets"
                                title="Search Event Contract markets"
                              >
                            ...
                            {eventMarketMenuOpen && (
                              <div className="event-market-results" id="event-market-results" role="listbox">
                                <div className="event-market-results-heading">
                                  <span>{eventSearchSubmitted
                                    ? `${eventTotalMatches}${eventHasMore ? '+' : ''} verified live contracts`
                                    : 'Search Webull Event Contracts'}</span>
                                  {eventLoading && (
                                    <span>Searching…</span>
                                  )}
                                </div>
                                {!eventSearchSubmitted && !eventLoading && (
                                  <div className="event-market-empty">
                                    Enter a contract title or symbol, then select Search.
                                  </div>
                                )}
                                {eventSearchSubmitted && !eventLoading && !eventMarkets.length && (
                                  <div className="event-market-empty">
                                    {eventMessage || 'No verified live contracts match this search.'}
                                  </div>
                                )}
                                {eventSearchSubmitted && eventMarkets.map((market) => (
                                  <button
                                    key={market.symbol}
                                    type="button"
                                    role="option"
                    ...
                    {(orderForm.type === 'LADDER' || orderForm.type === 'SYNTHETIC') && (
                      <div style={{ width: '100%', marginBottom: '14px' }}>
                        <LadderOrderConfig
                          ...
                          quoteAsset="USD"
                          quantityStep={selectedInstrumentType === 'CRYPTO' ? .00000001 : .00001}
                    ...
                    {orderFeedback.message && orderFeedback.type === 'success' && (
                        <div
                          role="alert"
                          aria-live="assertive"
                          className="modern-real-success"
                          style={{
                            marginTop: '12px',
                            padding: '12px 16px',
                            borderRadius: '8px',
                    ...
                    {isQuantMode ? (
                      <div className="modern-real-warning" style={{ marginTop: '12px', background: 'rgba(139, 92, 246, 0.15)', borderColor: 'rgba(139, 92, 246, 0.4)', color: '#c084fc' }}>
                        🤖 <strong>QUANTITATIVE STRATEGY MODE ACTIVE:</strong> The autonomous multi-asset algorithmic engine is managing orders and positions (${number(cashBalance)} USD cash / ${number(quantSummary?.total_equity ?? cashBalance)} equity). Manual order entry is locked to prevent interfering with model strategies.
                      </div>
                    ) : isTestMode ? (
                ...
                {/* Order Error Modal */}
                {orderFeedback.message && orderFeedback.type === 'error' && (
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: '20px' }}>
                    <div style={{ background: '#1e293b', borderRadius: '14px', padding: '0', maxWidth: '540px', width: '100%', border: '1px solid rgba(239, 68, 68, 0.3)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
                      <div style={{ background: 'rgba(239, 68, 68, 0.1)', padding: '20px 24px', borderBottom: '1px solid rgba(239, 68, 68, 0.2)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <span style={{ fontSize: '1.8rem' }}>⚠️</span>
                        <h3 style={{ margin: 0, color: '#ef4444', fontSize: '1.25rem' }}>Order Placement Error</h3>
                      </div>
                      <div style={{ padding: '24px' }}>
                        {(orderFeedback.agreementUrl || orderFeedback.errorCode === 'OPENAPI_FRACT_VERSION2_ACCOUNT_NOT_TRADE' || String(orderFeedback.message).includes('agreement/third-party')) ? (
                          <div>
                            <strong style={{ display: 'block', fontSize: '1.05rem', marginBottom: '8px', color: '#fbbf24' }}>
                              Webull Fractional Shares Trading Agreement Required
                            </strong>
                            <p style={{ margin: '0 0 12px 0', fontSize: '0.9rem', lineHeight: 1.45, color: '#f1f5f9' }}>
                            ...
                            <div style={{ background: 'rgba(0, 0, 0, 0.35)', borderRadius: '8px', padding: '12px 14px', marginBottom: '16px', fontSize: '0.85rem', lineHeight: 1.5, border: '1px solid rgba(255, 255, 255, 0.08)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '12px' }}>
                              {(orderFeedback.agreementUrl || (String(orderFeedback.message).match(/https?:\/\/[^\s"'\)\(\}]+/)?.[0])) && (
                                <a
                                  ...
                                  style={{
                                    display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '6px',
                                    background: '#f59e0b', color: '#0f172a', fontWeight: 600, fontSize: '0.88rem', textDecoration: 'none',
                              ...
                              <button
                                ...
                                style={{
                                  display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '6px',
                          ...
                          <div>
                            <p style={{ margin: '0 0 16px', color: '#f8fafc', fontSize: '1.05rem', lineHeight: 1.5 }}>
                              {orderFeedback.message}
                            </p>
                            {String(orderFeedback.message).includes('2% greater') && (
                              <div style={{ background: 'rgba(56, 189, 248, 0.1)', border: '1px solid rgba(56, 189, 248, 0.3)', padding: '12px 16px', borderRadius: '8px', color: '#e0f2fe', fontSize: '0.95rem' }}>
                                <strong style={{ color: '#38bdf8', display: 'block', marginBottom: '6px' }}>💡 How to fix this:</strong>
                                Reduce your order quantity slightly (e.g. by 2%) to leave room for the required market volatility buffer, or switch your order type to a <strong>Limit Order</strong> to use your full available balance.
                              </div>
                            )}
                          </div>
                        )}
                      </div>
                      <div style={{ padding: '16px 24px', background: 'rgba(0,0,0,0.2)', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'flex-end' }}>
                        <button
                          type="button"
                          onClick={() => setOrderFeedback({ type: '', message: '' })}
                          style={{ padding: '10px 24px', borderRadius: '8px', background: '#334155', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer', fontSize: '0.95rem' }}
                        >
                          Acknowledge
                        </button>
                      </div>
                ...
                {showConfirmModal && (
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
                    <div style={{ background: 'var(--card-bg, #1e293b)', borderRadius: '14px', padding: '28px', maxWidth: '680px', maxHeight: '88vh', overflowY: 'auto', width: '100%', border: '1px solid rgba(255,255,255,0.15)', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)' }}>
                      ...
                      <div style={{ background: 'rgba(0,0,0,0.25)', padding: '16px', borderRadius: '8px', marginBottom: '20px', display: 'grid', gap: '10px', fontSize: '0.95rem' }}>
                        ...
                        {['LADDER', 'SYNTHETIC'].includes(orderForm.type) && <SyntheticOrderReview review={webullTwoFactorOrderDetails().syntheticReview} />}
                ...
                {selectedInstrumentType === 'EVENT' ? (
                  <>
                    ...
                    <WebullHoldings
                      ...
                      mode={tradingMode} userId={user?.id}
                    />
                  </>
                ) : (
                  <WebullHoldings
            ...
            {/* POSITIONS TAB */}
            {activeTab === 'positions' && (
              <WebullPositions key={`${user?.id}:${tradingMode}`} positions={modeHoldings} mode={tradingMode} userId={user?.id} onSelectHolding={handleSelectHolding} onOpenEventPosition={handleOpenEventPosition} />
            )}
      ...
      <PaperDepositModal visible={showDepositModal} broker="Webull" balances={{ USD: cashBalance }}
        submitting={depositSubmitting} error={depositError} onClose={() => { setShowDepositModal(false); setDepositError(''); }}
        onDeposit={handleDepositFakeMoney} onReset={handleResetPaperAccount} />
...
function WebullHoldings({ holdings, instrumentType = 'EQUITY', onSelectHolding, onOpenEventPosition, mode, userId }) {
  return <div style={{ marginTop: 20 }}>
    <WebullPositions key={`${userId}:${mode}:${instrumentType}`} positions={holdings} mode={mode} userId={userId}
      initialAssetView={positionAssetType({ instrument_type: instrumentType })}
      onSelectHolding={onSelectHolding} onOpenEventPosition={onOpenEventPosition} />
  </div>;
}
