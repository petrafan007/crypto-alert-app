...
function SourceBadge({ order }) {
  ...
  return <span className="badge" style={{ background: webull ? 'rgba(96, 165, 250, .16)' : 'rgba(251, 191, 36, .16)', color: webull ? '#2563eb' : '#a16207' }}>{webull ? 'Webull · Read-only' : 'Binance.US'}</span>;
}

function OrderTable({ orders, open }) {
  if (!orders.length) return <div className="empty-state"><p>No {open ? 'open' : 'historical'} orders for the selected accounts.</p></div>;
  return <div className="table-container trading-table"><div className="order-table-scroll"><table><thead><tr><th>Date / Time</th><th>Account</th><th>Symbol</th><th>Side</th><th>Type</th><th>Quantity</th><th>Price</th><th>Filled</th><th>Status</th>{open && <th>Actions</th>}</tr></thead><tbody>{orders.map((order) => <tr key={`${order.source}-${order.id}`}><td>{timestamp(order.created_at)}</td><td><SourceBadge order={order} /></td><td>{order.symbol}</td><td>{displaySide(order.side)}</td><td>{displayType(order.order_type)}</td><td>{amount(order.quantity)}</td><td>{Number(order.price) > 0 ? `$${amount(order.price, 4)}` : 'Market'}</td><td>{amount(order.filled_quantity)}</td><td>{order.status}</td>{open && <td><span style={{ color: 'var(--text-secondary, #64748b)' }}>{isWebull(order) ? 'Manage in Webull' : 'Manage in Binance.US'}</span></td>}</tr>)}</tbody></table></div></div>;
}
...
export default function Orders() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // A Binance order can be visible in all-order history before the dedicated
      // open-order request responds, so safely use qualifying rows as a fallback.
  ...
  return <div className="trading-page" style={{ padding: '20px', maxWidth: '1500px', margin: '0 auto' }}><div className="trading-header" style={{ marginBottom: 18 }}><div><h1 style={{ fontSize: '2rem', margin: 0 }}>📋 Orders</h1><p style={{ margin: '6px 0 0', color: 'var(--text-secondary, #94a3b8)' }}>Open orders and history across Binance.US and Webull. Webull orders are read-only.</p></div><button type="button" className="btn btn-secondary" onClick={load}>🔄 Refresh</button></div>{notice && <div className="modern-real-warning" style={{ marginBottom: 16 }}>⚠️ {notice}</div>}<div className="trading-tabs"><button className={`tab-button ${activeTab === 'open' ? 'active' : ''}`} onClick={() => setActiveTab('open')}>⏳ <span className="tab-text">Open Orders</span>{activeOpenOrders.length > 0 && <span className="tab-badge">{activeOpenOrders.length}</span>}</button><button className={`tab-button ${activeTab === 'history' ? 'active' : ''}`} onClick={() => setActiveTab('history')}>📜 <span className="tab-text">Order History</span></button></div><div className="trading-content"><section className="order-history-container">{loading ? <div className="empty-state"><p>Loading combined orders…</p></div> : activeTab === 'open' ? <><h2>All Open Orders</h2><OrderTable orders={activeOpenOrders} open /></> : <><h2>All Order History</h2><OrderTable orders={paginatedHistory} /><Pagination page={historyPage} setPage={setHistoryPage} pageSize={historyPageSize} setPageSize={setHistoryPageSize} total={sortedHistory.length} /></>}</section></div></div>;
}
