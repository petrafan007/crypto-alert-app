        ...
        <div style={{
            ...
            <Section id="webull-openapi-setup" icon={<FaChartLine />} title="Webull OpenAPI Setup & Import">
                ...
                <ul style={{ paddingLeft: '20px', marginBottom: '16px', lineHeight: '1.8' }}>
                    ...
                    <li>For an equity or ETF, choose <strong>Only Regular Hours</strong>, <strong>Including Extended Hours</strong>, or <strong>Overnight Hours Only</strong>. Fractional stock/ETF orders are Market-only during Regular Hours and have a $5 minimum. Share quantities support up to five decimal places. For a sub-one-share Market Buy entered with the value or percentage controls, the ticket automatically uses Webull's native cash-amount order so the requested dollars are not converted into invalid share precision. Extended and Overnight sessions require whole shares.</li>
                    <li>For an Event Contract, choose a live Webull category and optionally filter by <strong>Duration / Frequency</strong>; these controls only load lightweight provider metadata and never start a contract scan. Enter words from the title, condition, event, series, or symbol, then select the magnifying-glass <strong>Search</strong> button to look up contracts currently open for a new position. Frequency choices load directly from Webull's Series catalog, and an explicit search loads matching Series without waiting for every market in the category. <strong>15 minutes</strong> is derived only for Webull series that explicitly identify that cadence, and <strong>Intraday</strong> combines those series with Webull's Hourly frequency. A 15-minute result shows the exact Eastern-time period derived from Webull's cutoff symbol. Records with a provider condition such as <strong>Target price: TBD</strong> are excluded from new-position results until Webull supplies that condition. Common forms such as <strong>Bitcoin fifteen minutes</strong>, <strong>BTC 15 min</strong>, and <strong>bitcoin 15m</strong> are treated consistently. Future and non-tradable contracts are excluded. The selected-contract brief explains what YES and NO mean, identifies fixed-target versus reference-price direction contracts, expresses allowed order prices in cents, and shows the contract cutoff and $1/$0 settlement. Catalog warnings remain with the submitted search instead of replacing live bid/ask information.</li>
            ...
            <Section id="two-factor-auth" icon={<FaShieldAlt />} title="Two-Factor Authentication (2FA)">
                ...
                <Tip>
                    Every six-digit field uses the standard browser one-time-code metadata. If the Bitwarden shield appears but its suggestion menu no longer opens when you focus the field, open the Bitwarden extension's Autofill settings, enable <strong>Show autofill suggestions on form fields</strong>, and turn off <strong>Display suggestions when icon is selected</strong> for immediate suggestions. Bitwarden—not the website—controls whether that extension popup opens automatically.
                </Tip>
            </Section>
            ...
            <Section id="trading-center" icon={<FaChartLine />} title="Trading Center">
                ...
                <ul style={{ paddingLeft: '20px', marginBottom: '16px', lineHeight: '1.9' }}>
                    ...
                    <li><strong>Open Orders tab</strong> — all pending real exchange orders plus any active Auto-Buy/Auto-Sell triggers. Every Open Orders table supports saved filtering, sortable headers, draggable column ordering, and show/hide column controls.</li>
                    <li><strong>Order History tabs</strong> — Binance.US, Webull, and combined Orders histories are paginated at 50 rows by default. Choose 20, 50, 100, or 200 rows per page. Every history table supports the same saved filtering, sorting, and column customization controls as Open Orders. Settings are stored per user and table and survive navigation, sign-out/sign-in, browser restarts, and upgrades. The Binance.US Origin badge distinguishes manual orders, Auto-Sell/Auto-Buy execution, and a manual order canceled by Auto-Sell to release its balance.</li>
