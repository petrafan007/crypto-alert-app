...
- [x] Implement Draggable Hexagonal Badge Marker.
  ...
  - [x] Enable dragging left/right along the X-axis to dynamically probe expected P- [ ] Enable dragging left/right along the X-axis to dynamically probe expected P&L.L.
...
- [x] Generate `Assumptions` Sheet.
  - [ ] Inputs: Baseline price, Strike, Premium, Multiplier, IV, Risk-Free Rate, Expiry, DTE.
  - [ ] Outputs: Total Premium at Risk, Expiration Breakeven, Breakeven % Change.
- [x] Generate `Option Price Matrix` Sheet.
  - [ ] Rows: 21 percentage steps (+10% down to -10%, with 0% midpoint).
  - [ ] Columns: Actual Calendar Dates across the contract duration.
  - [ ] Cells: Pre-populate with Excel-compatible Black-Scholes formulas (`NORMSDIST`) tailored for Calls/Puts.
- [x] Generate `P- [ ] Generate `P&L Matrix` SheetL Matrix` Sheet.
  - [ ] Mirror rows and columns from Option Price Matrix.
  - [ ] Calculate net dollar profit/loss per contract.
- [x] Generate `Combined View` Sheet.
  - [ ] Display aggregated string formatting: `Estimated Option Price / Net P&L`.
...
- [x] Pending Orders / Active Positions Integration.
  - [x] Display real-time liquidation P- [ ] Display real-time liquidation P&L (if closed immediately at current mark).L (if closed immediately at current mark).
  - [x] Add "Export Thesis" button for live scenario tracking.
- [ ] Hourly Background Thesis Updates.
...
- [ ] Create and publish GitHub release `v2.66.0`.
- [ ] Upgrade the personal instance at `/home/jcavallarojr/crypto_alert_app`.
